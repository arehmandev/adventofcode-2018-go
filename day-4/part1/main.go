...
func (p guardlist) Less(i, j int) bool {
	return p[i].totalSleep < p[j].totalSleep
}
...
func main() {
		...
		for _, entry := range guardObj.logs {
			...
			if strings.Contains(entry.message, "falls asleep") {
				...
				for index := startTime.Minute(); index < endTime.Minute(); index++ {
					minuteMap[index]++
					if minuteMap[index] > minuteMap[guardObj.maxSleepMin] {
	...
	fmt.Println(highestGuard.id, highestGuard.maxSleepMin, highestGuard.id*highestGuard.maxSleepMin)
}
